...
\section{Approach}\label{sec:approach}
  ...
  \subsection{Incremental approach}
    ...
    The lowest increment is the base model which preserves almost all the 
  ...
  \subsection{Base model}\label{sec:base-model}
    ...
    The UKL-optimized process is per default also fully pre-emptible.
    ...
    User-level processes are however not protected from the UKL-optimized one.
    ...
    \subsubsection{Address space layout}
      ...
      All other memory regions of the UKL-optimized process remain in user-space.
    ...
    \subsubsection{Transition on system calls}\label{sec:transition}
      ...
      \code{ukl\_entry\_syscall\_64}, that is used by the UKL-optimized process.
  ...
  \subsection{Implementation}
    ...
    \subsubsection{Base model}\label{sec:imp:base-model}
      ...
      Since the thread is a UKL-optimized it executes in kernel mode and thus the \code{CS} value
